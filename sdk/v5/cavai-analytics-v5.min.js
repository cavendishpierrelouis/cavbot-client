...
(function () {
 ...
 const PROJECT_KEY =
  ...
  (document.currentScript && document.currentScript.getAttribute("data-project-key")) ||
  "";
if (!PROJECT_KEY) return; // or console.warn + return
  ...
  function analyticsDisabled() {
    try {
      ...
      if (body && body.getAttribute("data-cavai-analytics") === "off") return true;
  ...
  async function flushQueue(reason) {
    ...
    try {
      const q = readQueue();
      if (!q.length) { __flushInFlight = false; __flushRerunRequested = false; return; }
      ...
      for (let i = 0; i < q.length; i += BATCH_SIZE) {
        ...
        if (!ok) { anyFailure = true; remaining.push.apply(remaining, chunk); }
      }
