(function () {
  ...
  function shouldLoadBrain(config) {
    if (!config) {
      return false;
    }
    if (config.widget === 'head' || config.widget === 'body') {
      return true;
    }
    return config.motionRequested;
  }
  ...
  function run() {
    ...
    config.motionEnabled = config.motionRequested && !reducedMotion;
    var snippet = SNIPPETS[config.widget] || SNIPPETS[DEFAULTS.widget];
    var cssPath = CSS_PATHS[config.widget] && CSS_PATHS[config.widget][config.style];
    ...
    onReady(function () {
      mountRoot(root, config, script);
    });
    if (shouldLoadBrain(config)) {
