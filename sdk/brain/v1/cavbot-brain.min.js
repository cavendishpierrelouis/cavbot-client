...
// ===== Boot Config (reads <script ... data-*> if present) =====
function readBootConfig() {
  if (typeof document === 'undefined') return {};

  // Best case: currentScript (works when not async-inserted oddly)
  var s = document.currentScript;

  // Fallback: find the script that loaded cavbot-brain
  if (!s) {
    var scripts = document.getElementsByTagName('script');
    for (var i = scripts.length - 1; i >= 0; i--) {
      var src = scripts[i].getAttribute('src') || '';
      if (src.indexOf('cavbot-brain') !== -1) { s = scripts[i]; break; }
    }
  }

  if (!s) return {};

  var api = s.getAttribute('data-api') || s.getAttribute('data-cavbot-api') || '';
  var key = s.getAttribute('data-project-key') || s.getAttribute('data-cavbot-project-key') || '';
...
// Optional: expose config for other modules (analytics runtime, console, etc.)
window.cavbotBrain = window.cavbotBrain || {};
window.cavbotBrain.config = window.cavbotBrain.config || {};
if (BOOT.apiUrl) window.cavbotBrain.config.apiUrl = BOOT.apiUrl;
if (BOOT.projectKey) window.cavbotBrain.config.projectKey = BOOT.projectKey;

// Optional compatibility with your earlier embed style:
if (BOOT.apiUrl && !window.CAVBOT_API_URL) window.CAVBOT_API_URL = BOOT.apiUrl;
if (BOOT.projectKey && !window.CAVBOT_PROJECT_KEY) window.CAVBOT_PROJECT_KEY = BOOT.projectKey;
window.__CAVBOT__ = window.__CAVBOT__ || {};
if (BOOT.projectKey) window.__CAVBOT__.sitePublicId = window.__CAVBOT__.sitePublicId || null; // optional
window.__CAVBOT__.env = window.__CAVBOT__.env || 'prod';
window.__CAVBOT__.sdkVersion = window.__CAVBOT__.sdkVersion || 'v1';
window.__CAVBOT__.siteOrigin = window.__CAVBOT__.siteOrigin || (typeof window !== 'undefined' ? window.location.origin : null);
